...
3. Ajax를 통해 list를 받아와서 $.pagingHash(list, listCount, pageCount, func) 또는 $.paging(list, listCount, pageCount, func)  함수 호출
$.pagingHash : 해시를 이용해 뒤로가기 시 이전 상태 유지
$.paging : 해시 이용 안함
...
(function ( $ ) {
    $.pagingHash = function(list, listCount, pageCount, func) {
    ...
    $.paging = function(list, listCount, pageCount, func) {
    	...
    	function pagingPrint(page) {
    		...
    		// 맨앞
    		if (startPage > 1) {
    			$("#pagingArea").append("<a class='icon' id='1'>&nbsp;<i class='fas fa-angle-double-left'></i>&nbsp;</a>");
    		}
    		
    		// 이전
    		if (startPage > 1) {
    			$("#pagingArea").append("<a class='icon prev' id='"+prevPage+"'>&nbsp;<i class='fas fa-angle-left'></i>&nbsp;</a>");
    		}
    		
    		// 페이지 번호
    		for (var i = startPage; i <= endPage; i++) {
    			if (i == page) {  // 현재 페이지는 class='currPage' 부여
    				$("#pagingArea").append("<a class='currPage num' id='"+i+"'>" + i + "</a>");
    			} else {
    				$("#pagingArea").append("<a class='num' id='"+i+"'>" + i + "</a>");
    			}
    		}
    		
    		// 다음
    		if (endPage != totalPage) {
    			$("#pagingArea").append("<a class='icon next' id='"+nextPage+"'>&nbsp;<i class='fas fa-angle-right'></i>&nbsp;</a>");
    		}
    		
    		// 맨뒤
    		if (endPage != totalPage) {
    			$("#pagingArea").append("<a class='icon' id='"+totalPage+"'>&nbsp;<i class='fas fa-angle-double-right'></i>&nbsp;</a>");
    		}
    	};
    };
